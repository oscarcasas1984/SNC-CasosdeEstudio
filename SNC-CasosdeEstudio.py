...
def obtener_nombre_base(solucion):
    """
    Extrae el nombre base de una solución, removiendo sufijos como ' - 1', '(2)', etc.
    Ejemplo: 'Manglares (2)' → 'Manglares'
    """
    import re
    return re.sub(r"[\s\-]*\(?\d+\)?$", "", solucion).strip()
...
#Función de Expandir Gastos
def expandir_gastos(nombre_base, n_anios):
    """
    Expande los gastos definidos en gastos_detallados_por_solucion[nombre_base]
    en un DataFrame con una fila por año en el que se ejecuta cada gasto.
    """
    registros = []
    gastos = gastos_detallados_por_solucion.get(nombre_base, [])

    for item in gastos:
        inicio = item["año_inicio"]
        fin = min(inicio + item["duracion"], n_anios)
        frecuencia = item["frecuencia"]

        if frecuencia == 0:  # Solo una vez
            registros.append({"Año": inicio, "Concepto": item["concepto"], "Monto": item["monto"]})
        else:
            for año in range(inicio, fin):
                if (año - inicio) % frecuencia == 0:
                    registros.append({"Año": año, "Concepto": item["concepto"], "Monto": item["monto"]})

    return pd.DataFrame(registros)

#Función de Calculo de VPN
def calcular_vpn_solucion(
    ...
    gastos_especificos_vector=None  # 👈 Nuevo argumento opcional
):
    if gastos_especificos_vector is None:
        gastos_especificos_vector = np.zeros(n_anios_default)
    ...
    for anio in range(1, dur):
        ...
        gasto_extra = (
            gastos_adicionales[anio]
            + monitoreo_en_campo[anio]
            + gastos_especificos_vector[anio]  # 👈 Se suma aquí
        )
...
# --- TÍTULO DE CONTROL ---
st.title("🧪 Laboratorio de Modelación de SNC")
...
<div style="background-color: #F8F9FA; padding: 20px; border-radius: 10px; margin-bottom: 20px;">
    ...
    <div style="font-size: 16px; color: #666666;">Modelo interactivo para análisis de Soluciones Naturales del Clima</div>
</div>
""", unsafe_allow_html=True)
...
# --- CONFIGURACIÓN GENERAL DEL MODELO (Disponible para Excel o Sidebar) ---

# 💡 Este diccionario contiene los parámetros técnicos y económicos de cada tipo de solución base.
# Puedes agregar nuevas soluciones siguiendo esta estructura:
# "Nombre": {
#     "captura": valor (si es constante),
#     "costo": USD/ha-año,
#     "duracion": años,
#     "capex": USD total,
#     "tipo_captura": "constante" | "lineal" | "sigmoidal",
#     "tipo_sn": "restauracion" | "degradacion",
#     otros parámetros según tipo de curva
# }
...
soluciones_predeterminadas = {
    ...
    "Bosque Seco Tropical": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Corales": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Agroforestería con Cacao": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Bosque de Galería": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Turberas Andinas": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Restauración de Pastos Degradados": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Reforestación Productiva de Zonas ECP": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    #Poblado para Caso de Estudio
    "Manglar Degradación Evitada": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 1636122, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Manglar Degradación Evitada (2)": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Bosque Húmedo Degradación Evitada": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Bosque Húmedo Degradación Evitada (2)": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    ...
    "Humedal Degradación Evitada": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Humedal Degradación Evitada (2)": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Humedal Degradación Evitada (3)": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Pastos Degradación Evitada": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    "Pastos Degradación Evitada (2)": {"captura": 5.5, "costo": 55, "duracion": 30, "capex": 370, "tipo_captura": "constante", "tipo_sn": "degradacion"},
    # Ejemplos de curvas no constantes
...
# 💡 Este diccionario contiene los gastos detallados por tipo de solución.
# Cada entrada es una lista de ítems con su año de inicio, frecuencia, duración y monto.
# Puedes duplicar la estructura para nuevos tipos copiando un bloque y modificando los valores.

gastos_detallados_por_solucion = {
    "Manglares": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 50000},
        {"concepto": "Monitoreos Campo (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 10000},
        {"concepto": "Costos de Transacción (USD)", "año_inicio": 1, "frecuencia": 5, "duracion": 30, "monto": 5000}
    ],
    "Pastos Marinos": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    ...
    #Poblado para Caso de Estudio ICPET
    "Manglar Degradación Evitada": [
        {"concepto": "Salvaguardas (USD)", "año_inicio": 0, "frecuencia": 1, "duracion": 3, "monto": 25000},
        {"concepto": "Estudios Previos (USD)", "año_inicio": -2, "frecuencia": 1, "duracion": 2, "monto": 5000},
        {"concepto": "Nuevos Sistemas Productivos (USD)", "año_inicio": 0, "frecuencia": 1, "duracion": 10, "monto": 101941},
        {"concepto": "Imprevistos (USD)", "año_inicio": 0, "frecuencia": 1, "duracion": 15, "monto": 3000},
        {"concepto": "Monitoreos Campo", "año_inicio": 3, "frecuencia": 1, "duracion": 30, "monto": 40800},
        {"concepto": "Costos de Transacción (USD)", "año_inicio": 1, "frecuencia": 3, "duracion": 20, "monto": 30000}
    ],
    "Manglar Degradación Evitada (2)": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Bosque Húmedo Degradación Evitada": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Bosque Húmedo Degradación Evitada (2)": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Páramo Degradación Evitada": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Páramo Degradación Evitada (2)": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Humedal Degradación Evitada": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Humedal Degradación Evitada (2)": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Humedal Degradación Evitada (3)": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Pastos Degradación Evitada": [
        {"concepto": "Estudios Previos (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 40000},
        {"concepto": "OPEX Sistemas Productivos (USD)", "año_inicio": 1, "frecuencia": 1, "duracion": 30, "monto": 15000}
    ],
    "Pastos Degradación Evitada (2)": [
        {"concepto": "Salvaguardas (USD)", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 30000},
        {"concepto": "Monitoreos Campo (USD)", "año_inicio": 1, "frecuencia": 3, "duracion": 30, "monto": 8000},
        {"concepto": "Otros 1", "año_inicio": 2, "frecuencia": 10, "duracion": 30, "monto": 6000}
    ]
}

# Seguir este ejemplo para agregar más soluciones:
# "Bosque Seco Tropical": [
#     {"concepto": "Estudios Previos", "año_inicio": 0, "frecuencia": 0, "duracion": 1, "monto": 35000},
#     {"concepto": "Monitoreos Campo", "año_inicio": 2, "frecuencia": 2, "duracion": 30, "monto": 12000}
# ]
...
if not df_soluciones.empty:
    ...
    for _, sol in df_soluciones.iterrows():
        try:
            ...
            # --- SENSIBILIDAD: variación de captura por ha ±20%
            carbonos = {}
            for esc, mult in zip(["Bajo (-20%)", "Medio", "Alto (+20%)"], [0.8, 1.0, 1.2]):
                if tipo_captura == "constante":
                    cap_ha_esc = np.full(dur, sol["Captura por ha (tCO2e)"] * mult)
                elif tipo_captura == "lineal":
                    cap_ha_esc = np.linspace(cap_ini * mult, cap_fin * mult, dur)
                elif tipo_captura == "sigmoidal":
                    cap_ha_esc = (cap_max * mult) / (1 + np.exp(-k * (x_vals - x0)))
                else:
                    cap_ha_esc = np.full(dur, sol["Captura por ha (tCO2e)"] * mult)

                cap_ha_esc = np.pad(cap_ha_esc, (0, n_anios_default - len(cap_ha_esc)), constant_values=0)

                if tipo_sn == "degradacion":
                    captura_anual_esc = area_por_anio * perdida_pct * cap_ha_esc * salv
                else:
                    captura_anual_esc = area_por_anio * cap_ha_esc * salv

                carbonos[esc] = sum(captura_anual_esc)

            st.write(f"✅ Procesando solución: {sol['Solución']} — Captura total caso base: {captura_total:.2f} tCO2e")

            resultados.append({
                "Solución": sol["Solución"],
                "Carbono Total (tCO2e)": captura_total,
                "Escenario Bajo (-20%)": carbonos["Bajo (-20%)"],
                "Escenario Medio": carbonos["Medio"],
                "Escenario Alto (+20%)": carbonos["Alto (+20%)"]
            })

        except Exception as e:
            st.error(f"❌ Error al procesar la solución: {sol.get('Solución', 'Desconocida')}")
            st.error(f"🧨 Detalles del error: {e}")

    df_resultados_carbono = pd.DataFrame(resultados)
    #st.write("🧪 Columnas en df_resultados_carbono:", df_resultados_carbono.columns.tolist())

# --- Gráfica 1: Carbono Total por Solución (Escenario Medio / Base)
if 'df_resultados_carbono' in locals() and not df_resultados_carbono.empty:
    st.subheader("Carbono Total por Solución – Escenario Base")

    fig_base = go.Figure()

    fig_base.add_trace(go.Bar(
        x=df_resultados_carbono["Solución"],
        y=df_resultados_carbono["Escenario Medio"],
        name="Escenario Base (0%)",
    ...
    fig_base.update_layout(
        ...
        yaxis_title="Carbono Total (tCO₂e)",
        title="Carbono Total por Solución – Escenario Base",
        plot_bgcolor="white",
        paper_bgcolor="white"
    )
...
# --- Gráfica 2: Sensibilidad del Carbono Total (±20% Captura/ha)
    st.subheader("Sensibilidad del Carbono Total por Solución (±20% Captura/ha)")

    fig_sens = go.Figure()

    fig_sens.add_trace(go.Bar(
        x=df_resultados_carbono["Solución"],
        y=df_resultados_carbono["Escenario Bajo (-20%)"],
        name="Escenario Bajo (-20%)",
        marker_color="#d62728"  # Rojo ejecutivo
    ))

    fig_sens.add_trace(go.Bar(
        x=df_resultados_carbono["Solución"],
        y=df_resultados_carbono["Escenario Medio"],
        name="Escenario Medio (0%)",
        marker_color="#1f77b4"
    ))

    fig_sens.add_trace(go.Bar(
        x=df_resultados_carbono["Solución"],
        y=df_resultados_carbono["Escenario Alto (+20%)"],
        name="Escenario Alto (+20%)",
        marker_color="#2ca02c"  # Verde potente
    ))
    ...
    fig_sens.update_layout(
        ...
        xaxis_title="Solución",
        yaxis_title="Carbono Total (tCO₂e)",
        title="Sensibilidad del Carbono Total por Solución (±20% Captura/ha)",
        plot_bgcolor="white",
        paper_bgcolor="white",
        font=dict(size=14),
        height=500,
...
if not df_acumulada.empty and "Año" in df_acumulada.columns:
    ...
    st.warning("⚠️ Agrega Datos al Modelo")

# === Cálculo financiero ===

# Definir gastos adicionales comunes por solución (se mantienen)
gastos_adicionales_comunes = [
...
for _, sol in df_soluciones.iterrows():
    # Obtener nombre base para acceder a gastos detallados por tipo
    nombre_base = obtener_nombre_base(sol["Solución"])
    df_gastos_especificos = expandir_gastos(nombre_base, n_anios_default)

    # Crear vector de gastos anuales específicos
    gastos_especificos_vector = np.zeros(n_anios_default)
    for _, fila in df_gastos_especificos.iterrows():
        anio = int(fila["Año"])
        if 0 <= anio < n_anios_default:
            gastos_especificos_vector[anio] += float(fila["Monto"])

    # Calcular resultados financieros y de captura
    vpn, flujo_proyecto, captura_anual, area_por_anio, costo_anual, monitoreo_en_campo, gastos_adicionales = calcular_vpn_solucion(
        ...
        crecimiento_precio_carbono,
        crecimiento_ingreso_encadenado,
        gastos_especificos_vector  # 👈 nuevo argumento
    )
    ...
    df_tabla_financiera = pd.DataFrame({
        ...
        "Gastos específicos (USD)": np.round(gastos_especificos_vector, 2),  # 👈 nuevo en tabla
        "Flujo neto (USD)": np.round(flujo_proyecto, 2)
    })

    with st.expander(f"📊 Flujo de Caja Año a Año – {sol['Solución']}", expanded=False):
...
# Mostrar
with st.expander("🔎 Resumen Flujo de Caja Acumulado (USD)", expanded=False):
...
for _, sol in df_soluciones.iterrows():
    try:
        ...
        st.markdown(f"#### Resumen Técnico: {sol['Solución']}")
        st.code(f"""
...
st.markdown("## Sensibilidad del VPN por SNC")
...
for _, sol in df_soluciones.iterrows():
    ...
    # Calcular escala visual centrada en 0
    z_min = np.nanmin(matriz_vpn_individual)
    z_max = np.nanmax(matriz_vpn_individual)
    rango_abs_max = max(abs(z_min), abs(z_max))
    z_min_final = -rango_abs_max
    z_max_final = rango_abs_max
    ...
    with st.expander(f"Sensibilidad del VPN – {nombre_solucion}", expanded=False):
        st.plotly_chart(fig, use_container_width=True)  

# --- Heatmap de Sensibilidad del VPN---
rango_precio = np.arange(5, 51, 5)
rango_descuento = np.arange(1, 22, 1)
...
# Calculamos extremos de la matriz
z_min = np.min(matriz_vpn)
z_max = np.max(matriz_vpn)

# Aseguramos simetría en la escala si hay negativos
rango_abs_max = max(abs(z_min), abs(z_max))
z_min_final = -rango_abs_max
z_max_final = rango_abs_max
#st.write(f"🌡 z_min: {z_min}, z_max: {z_max}")
fig_heat = go.Figure()
...
fig_heat.add_trace(go.Heatmap(
    ...
    x=rango_precio,
    y=rango_descuento,
    colorscale="BrBg",    # diverging: rojo-amarillo-verde
    zmin=z_min_final,       # forzamos que el colorbar arranque en negativo
    zmax=z_max_final,
    zmid=0,                 # centro visual en 0
    colorbar=dict(
        title="VPN (USD)",
        tickformat=",",
        exponentformat="none"
...
# 🔧 Aquí agregamos los rótulos de ejes
fig_heat.update_layout(
    title="📊 Sensibilidad del VPN - Portafolio Integrado",
    xaxis_title="Precio del carbono (USD/tCO₂e)",
    yaxis_title="Tasa de descuento (%)"
)
#st.subheader("🔥 Heatmap con Escala Centrada en 0")
st.plotly_chart(fig_heat, use_container_width=True)

#DESDE ACÁ, SEGMENTO PARA COLAPSAR
#ZONAS DE PRUEBAS CUIDADO



#CIERRE DE LA PRUEBA
st.markdown("### Resumen Técnico del Caso Base ")
...
for _, sol in df_soluciones.iterrows():
    ...
    with st.expander(f"Detalles técnicos de: {sol['Solución']}", expanded=False):
        st.code(f'''
VPN individual: {vpn}
Área total (ha): {sol["Área (ha)"] * multiplicador_area}
Ingreso encadenado (USD/año): {sol.get("Ingreso Encadenado (USD/año)", 0)}
...
st.markdown("""---""")
st.markdown("""
<div style='text-align: center; color: #444444; font-size: 14px;'>
